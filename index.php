<?php include('header.php');?>
        <div id="mid">
           
                <div class="container">
                    <div class="row">
                        <div class="col-lg-6">
                            <h3 style="padding-top:60px;
                            margin-bottom: 0.5rem;
                            font-weight: 500;
                            line-height: 1.2; color: black;">Welcome AssignmentHelp</h3>
                            <p class=" ">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quam exercitationem assumenda facilis tempore sed dolorum, necessitatibus amet sunt dolores! Maiores pariatur dignissimos quis facere quae cupiditate, quaerat ipsa dolorem aliquam.</p>
                            <button class="btn text-light" style="background-color: #0d6efd;">Enqury</button>
                        </div>
                        <div class="col-lg-6"></div>
                    </div>
                
            </div>
        </div>
        <section>
            <h3 style="padding-top:60px;
            margin-bottom: 0.5rem;
            font-weight: 500;
            line-height: 1.2; color: #0d6efd; text-align: center;" class="revealstop1  mt-5">Our Team</h3>
            <p class=" text-center">Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo laborum dolores provident adipisci quos ipsum odio animi consectetur maxime </p>
            <div class="jumbotron mt-5">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-2">
                            <div class="card1">
                                
                            
                            </div>
                        </div>
                        <div class="col-lg-2">
                            <div class="card1"></div>
                        </div>
                        <div class="col-lg-2">
                            <div class="card1"></div>
                        </div>
                        <div class="col-lg-2">
                            <div class="card1"></div>
                        </div>
                        
                        <div class="col-lg-2">
                            <div class="card1"></div>
                        </div>
                        <div class="col-lg-2">
                            <div class="card1"></div>
                        </div>
                        
                    </div>
                </div>
            </div>


        </section>
    

        <div class="container mt-5">
            <div class="row mt-5">
                <div class="col-lg-6">
                    <img style="margin-top: 60px;" src="https://www.seekpng.com/png/detail/824-8247150_click-happy-customers-cartoon.png" height="300px" width="400px" alt="">
                </div>
                <div class="col-lg-6">
                    <h3 style="padding-top:90px;
                            margin-bottom: 0.5rem;
                            font-weight: 500;
                            line-height: 1.2; color: #0d6efd;" class="revealstop1 ">Clients</h3>
                             <p class="  revealsleft active ">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quam exercitationem assumenda facilis tempore sed dolorum, necessitatibus amet sunt dolores! Maiores pariatur dignissimos quis facere quae cupiditate, quaerat ipsa dolorem aliquam. Lorem ipsum dolor sit amet consectetur adipisicing elit. Ab, sed laborum eos nobis in illum, odio placeat temporibus deleniti, eaque excepturi nisi quaerat. Quas eveniet error in beatae labore tempore. Lorem ipsum dolor sit amet consectetur adipisicing elit. Earum unde tempore consequuntur sapiente! Fuga veritatis quasi dicta molestias quam architecto rerum ut, in soluta nobis vel ullam corporis ducimus esse.</p>
                </div>
            </div>
        </div>


        <div class="container mt-5">
            <div class="row mt-5">
                <div class="col-lg-6">
                    <h3 style="padding-top:40px;
                    margin-bottom: 0.5rem;
                    font-weight: 500;
                    line-height: 1.2; color: #0d6efd; text-align: center;" class="revealstop1  ">Our Services</h3>
                    <div class="">
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="card11"></div>
                                </div>
                                <div class="col-lg-6 mt-5">
                                    <div class="card11"></div>
                                </div>
                            </div>
                        </div>
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-lg-6 ">
                                    <div class="card11"></div>
                                </div>
                                <div class="col-lg-6 mt-5">
                                    <div class="card11"></div>
                                </div>
                            </div>
                        </div>
                    </div>


                </div>
                <div class="col-lg-6">
                    <div class="col-lg-6">
                        <img style="margin-top: 60px;" src="https://www.seekpng.com/png/detail/824-8247150_click-happy-customers-cartoon.png" height="300px" width="400px" alt="">
                    </div>
                </div>
            </div>
        </div>

        

        
    
</body>
</html>